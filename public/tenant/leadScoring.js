...
(function () {
  ...
  function _scoreBadgeHtml(score, category) {
    ...
    return `<span style="display:inline-flex;align-items:center;gap:4px;background:${c.bg};color:${c.text};padding:2px 8px;border-radius:99px;font-size:.75rem;font-weight:600" title="Smart Score">🎯 ${score} · ${category || ''}</span>`;
  }
  ...
  function _renderHotLeadsDashboard(view) {
    ...
    (async () => {
      const enabled = await _probeEnabled();
      if (!enabled) {
        view.innerHTML = '<div style="padding:40px;text-align:center;color:#94a3b8"><h2>Lead Scoring is not enabled</h2><p>An admin must enable Smart Lead Scoring in Settings before this dashboard is populated.</p></div>';
        return;
      }
      ...
      header.innerHTML = `
        <div>
          ...
          <div style="font-size:.85rem;opacity:.75;margin-top:2px">Leads ranked by Smart Score — call the top ones first</div>
        </div>
  ...
  function _renderSettings(view) {
    ...
    (async () => {
      ...
      card.innerHTML = `
        <h2 style="margin:0 0 4px;font-size:1.3rem;color:#0f172a">🎯 Smart Lead Scoring Settings</h2>
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:${status?.enabled ? '#dcfce7' : '#fef3c7'};border-radius:6px;margin-bottom:20px">
          ...
          <label for="lsEnabled" style="font-weight:600;color:#0f172a;cursor:pointer">Enable Smart Lead Scoring</label>
