  ...
  return h('li', { class: 'rec-item rec-placeholder' },
    ...
    h('div', { class: 'ai-summary-card sample' },
      ...
      h('div', { class: 'ai-summary-text' },
        'Customer wants a Demat + trading account. Asked about brokerage, F&O margins and IPO subscription process. Already has an account with Zerodha but unhappy with charges.'
      ),
      h('div', { class: 'ai-insight' }, '💡 Hot prospect — comparing brokers, ready to switch this week.'),
      h('div', { class: 'ai-actions' },
        h('div', { class: 'ai-actions-title' }, '✓ Action items'),
        h('div', { class: 'ai-action-item' }, '• Share brokerage comparison PDF on WhatsApp'),
        h('div', { class: 'ai-action-item' }, '• Send IPO subscription guide'),
        h('div', { class: 'ai-action-item' }, '• Schedule onboarding call Friday 4 PM')
      ),
...
function ratingStars(recId, currentRating, aiRating, onChange) {
  ...
  function paintHover(n) { stars.forEach((s, idx) => s.classList.toggle('filled', idx < n)); }
  paint();
  if (currentRating != null) {
    const clear = h('button', { class: 'btn xs ghost rating-clear', title: 'Clear rating', onclick: async () => {
      try { await api('api_recording_rate', recId, null, ''); currentRating = null; paint(); toast('Rating cleared'); }
      catch (e) { toast(e.message, 'err'); }
    } }, '✕');
    wrap.appendChild(clear);
  }
  if (aiRating != null && currentRating == null) {
    wrap.appendChild(h('span', { class: 'rating-ai-hint', title: 'AI suggested rating' }, '🤖 ' + aiRating + '/5'));
  }
  return wrap;
}
...
/**
 * Agent assignment picker for the chat header.
 */
...
function buildWaCompose(phone, onSent) {
  ...
  fileInput.addEventListener('change', async () => {
    ...
    if (f.size > 25 * 1024 * 1024) {
      toast('File too large — upload limit is 25 MB', 'err');
      fileInput.value = '';
      return;
    }
...
async function wbChat() {
  ...
  async function renderThreadList() {
    ...
    if (fp === lastThreadsFingerprint) return;
  ...
  async function openThread(phone) {
    ...
    right.appendChild(h('div', { class: 'wb-chat-head', style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '.5rem', flexWrap: 'wrap' } },
      h('div', { style: { display: 'flex', alignItems: 'center', gap: '.5rem' } },
        h('button', { class: 'wb-chat-back', title: 'Back to list', onclick: backToList }, '← Back'),
...
async function wbAssignSettings() {
  ...
  wrap.appendChild(h('p', { class: 'muted' },
    ...
    'Rules only apply when a chat doesn\'t already have an agent — once a chat is assigned, the assignment sticks until someone changes it.'
  ));
  ...
  wrap.appendChild(h('details', { style: { marginTop: '1.25rem' } },
    ...
    h('ul', { style: { paddingLeft: '1.2rem', lineHeight: '1.6' } },
      h('li', {}, h('b', {}, 'Lead owner'), ' — chat goes to whoever owns the lead with that phone number. If the lead has no owner, the chat stays unassigned (admin only).'),
      h('li', {}, h('b', {}, 'Round-robin'), ' — chats are distributed evenly across the agent pool, remembering who got the last one.'),
      h('li', {}, h('b', {}, 'Least-busy'), ' — counts each agent\'s active chats and gives the new one to whoever has the fewest.'),
      h('li', {}, h('b', {}, 'Manual'), ' — chats stay unassigned until someone picks an agent from the header dropdown.'),
      h('li', {}, 'Already-assigned chats are never re-routed by these rules.')
    )
  ));
...
VIEWS.callratings = async (view) => {
  ...
  async function load() {
    ...
    try {
      ...
      if (rows && rows.error) { out.innerHTML = '<div class="ai-error">' + esc(rows.error) + '</div>'; return; }
      if (!rows || rows.length === 0) { out.innerHTML = '<div class="muted">No call recordings in this range.</div>'; return; }
      out.innerHTML = '';
      const tbl = h('table', {},
        h('thead', {}, h('tr', {},
          h('th', {}, 'Rep'),
      ...
      tbl.appendChild(tb);
      rows.forEach(r => {
        const stars = (n, color) => n == null ? h('span', { class: 'muted' }, '—')
          : h('span', { style: 'color:' + (color || '#f59e0b'), fontWeight: 600 },
            n.toFixed(1) + ' ' + '★'.repeat(Math.round(n)) + '☆'.repeat(5 - Math.round(n)));
        tb.appendChild(h('tr', {},
          h('td', {}, h('b', {}, r.user_name || '—'), r.user_role ? h('span', { class: 'muted' }, ' · ' + r.user_role) : null),
  ...
  view.appendChild(h('div', { class: 'toolbar' },
    h('span', {}, 'From'), fromInp, h('span', {}, 'to'), toInp,
    h('span', {}, 'User'), userSel,
    h('button', { class: 'btn primary', onclick: load }, '🔎 Apply')
  ));
  view.appendChild(h('div', { class: 'card' },
    h('h3', {}, '⭐ Call ratings — rep-wise'),
    h('p', { class: 'muted' }, 'Manual ratings are 1-5 stars set by the rep or manager on each recording. AI-suggested ratings come from Gemini.'),
    out
  ));
  load();
};
