  ...
  list.forEach(a => {
    view.appendChild(h('div', { class: 'card' },
      h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
        h('div', {},
          h('h3', {}, a.title, ' ',
            h('span', { class: 'tag ' + (a.level === 'critical' ? 'err' : a.level === 'warn' ? 'warn' : 'info') }, a.level),
            ' ', h('span', { class: 'tag', style: { background: a.display_mode === 'popup' ? '#ede9fe' : '#e2e8f0', color: a.display_mode === 'popup' ? '#6d28d9' : '#475569' } }, a.display_mode === 'popup' ? '🪧 pop-up' : '📢 banner')),
...
function editAnnouncement(a) {
  ...
  // ---- Popup-only options (shown when display_mode = popup) ----
  const popupBox = h('div', { style: {
    border: '1px dashed #c7d2fe', borderRadius: '10px', padding: '12px',
    marginTop: '4px', background: '#f8faff',
    display: (a.display_mode === 'popup') ? 'block' : 'none' } },
    h('div', { style: { fontWeight: 700, fontSize: '.82rem', color: '#4338ca', marginBottom: '8px' } },
      '🎨 Pop-up design'),
    h('div', { class: 'field' },
      h('label', {}, 'Image URL (optional) — shown big, clickable'),
      h('input', { name: 'image_url', value: a.image_url || '', placeholder: 'https://…/banner.png' })),
    h('div', { class: 'field' },
      h('label', {}, 'Clickable link URL (whole popup / button opens this)'),
      h('input', { name: 'link_url', value: a.link_url || '', placeholder: 'https://…' })),
    h('div', { class: 'field' },
      h('label', {}, 'Button text'),
      h('input', { name: 'cta_text', value: a.cta_text || '', placeholder: 'Learn more' })),
    h('div', { style: { display: 'flex', gap: '10px', flexWrap: 'wrap' } },
      h('div', { class: 'field', style: { flex: 1 } }, h('label', {}, 'Background'),
        h('input', { name: 'bg_color', type: 'color', value: a.bg_color || '#4f46e5' })),
      h('div', { class: 'field', style: { flex: 1 } }, h('label', {}, 'Text'),
        h('input', { name: 'text_color', type: 'color', value: a.text_color || '#ffffff' })),
      h('div', { class: 'field', style: { flex: 1 } }, h('label', {}, 'Button/accent'),
        h('input', { name: 'accent_color', type: 'color', value: a.accent_color || '#f59e0b' }))),
    h('label', { style: { display: 'flex', gap: '8px', alignItems: 'center', fontSize: '.85rem' } },
      h('input', { name: 'dismissible', type: 'checkbox', value: '1', checked: a.dismissible === 0 ? null : true }),
      'Let users dismiss it (recommended)')
  );

  const modeSel = h('select', { name: 'display_mode', onchange: () => {
    popupBox.style.display = (modeSel.value === 'popup') ? 'block' : 'none';
  } },
    h('option', { value: 'banner', selected: a.display_mode !== 'popup' ? true : null }, 'Banner (top strip)'),
    h('option', { value: 'popup', selected: a.display_mode === 'popup' ? true : null }, 'Pop-up (center modal)'));

  const form = h('form', { onsubmit: async ev => {
    ev.preventDefault();
    const fd = new FormData(form);
    const payload = Object.fromEntries(fd);
    payload.dismissible = fd.get('dismissible') ? 1 : 0;   // unchecked box omits the key
    if (a.id) payload.id = a.id;
    payload.is_active = 1;
    try { await api('api_saas_announcements_save', payload); toast('Saved'); m.remove(); navigate('announcements'); }
    catch (e) { toast(e.message, 'err'); }
  } },
    h('div', { class: 'field' }, h('label', {}, 'Display as'), modeSel),
    h('div', { class: 'field' }, h('label', {}, 'Title *'), h('input', { name: 'title', required: true, value: a.title || '' })),
    h('div', { class: 'field' }, h('label', {}, 'Body (HTML allowed — banner needs this; popup can be image-only)'),
      h('textarea', { name: 'body', rows: 4 }, a.body || '')),
    h('div', { class: 'field' }, h('label', {}, 'Level / colour'),
      h('select', { name: 'level' },
        ...['info', 'warn', 'critical', 'new_feature'].map(v => h('option', { value: v, selected: a.level === v ? true : null }, v)))),
    popupBox,
    h('button', { type: 'submit', class: 'btn', style: { marginTop: '8px' } }, 'Save & send')
  );
  card.appendChild(form);
  m.appendChild(card);
  document.body.appendChild(m);
...
VIEWS.signup_requests = async (view) => {
  ...
  (function renderSubmittedBySummary() {
    ...
    // SR_SUMMARY_MONTH_DROPDOWN_v1 — filter the summary to a single month.
    const monthSel = h('select', { style: { padding: '.3rem .5rem' }, onchange: ev => { sumState.month = ev.target.value; render(); } },
    ...
    card.appendChild(h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '.5rem', marginBottom: '.5rem' } },
      ...
      h('div', { style: { display: 'flex', gap: '.4rem', alignItems: 'center' } }, monthSel, statusSel, csvBtn)
    ));
    const body = h('div', {});
    card.appendChild(body);
    ...
    let _last = null;
    // Rows to show given the current month filter, with recomputed totals.
    function displayed() {
      const all = (_last && _last.rows) || [];
      const rows = sumState.month ? all.filter(x => x.month === sumState.month) : all;
    ...
    // Rebuild the month dropdown from the distinct months in the data.
    function syncMonths() {
      const months = Array.from(new Set(((_last && _last.rows) || []).map(x => x.month))).sort().reverse();
      const cur = sumState.month;
      monthSel.replaceChildren(
        h('option', { value: '' }, 'All months'),
        ...months.map(m => h('option', Object.assign({ value: m }, m === cur ? { selected: 'selected' } : {}), monthLabel(m)))
      );
      if (cur && !months.includes(cur)) { sumState.month = ''; monthSel.value = ''; }
    }
    function render() { body.replaceChildren(table(displayed())); }
    function table(r) {
      const rows = (r && r.rows) || [];
    ...
    csvBtn.onclick = () => {
      const d = displayed();
      if (!d.rows.length) { toast('Nothing to export', 'err'); return; }
      const esc = v => '"' + String(v == null ? '' : v).replace(/"/g, '""') + '"';
      const lines = [['Submitted by', 'Month', 'Signups', 'Amount', 'Paid', 'Balance'].join(',')];
      d.rows.forEach(x => lines.push([esc(x.rep), esc(monthLabel(x.month)), x.count, x.amount, x.paid, x.balance].join(',')));
      const t = d.totals;
      lines.push(['Grand total', '', t.count || 0, t.amount || 0, t.paid || 0, t.balance || 0].join(','));
      const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'signup-submittedby-summary' + (sumState.month ? '-' + sumState.month : '') + '.csv'; a.click();
    };
    async function load() {
      body.replaceChildren(h('div', { class: 'muted', style: { padding: '.5rem' } }, 'Loading…'));
      try {
        _last = await api('api_saas_sr_submittedby_summary', { status: sumState.status || undefined });
        syncMonths();
        render();
      } catch (e) { body.replaceChildren(h('div', { class: 'error-box' }, e.message)); }
    }
