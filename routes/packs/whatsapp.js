...
async function api_wapack_order_setStatus(token, args) {
  ...
  // INVENTORY_v1 — on return/cancel, add the stock back ONCE (only when inventory ON).
  const _invStore = await _getStore();
  if ((status === 'returned' || status === 'cancelled') && Number(o.stock_restored) !== 1 && _invStore && Number(_invStore.inventory_on) === 1) {
...
function _storeHtml(store, prods, slug) {
  ...
  return '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1">' +
    ...
    'h+="<div style=\\"color:var(--wd);font-weight:800;font-size:19px;margin:4px 0 8px\\">"+inr(p.price)+"</div>";' +
    'h+=(p.desc?("<div style=\\"color:#475569;font-size:14px;line-height:1.55;white-space:pre-wrap\\">"+esc(p.desc)+"</div>"):"<div style=\\"color:#94a3b8;font-size:12.5px\\">No description provided.</div>");' +
    'h+="<div class=qtyrow style=\\"justify-content:center;gap:20px;margin:16px 0\\"><button class=qbtn onclick=\\"chg("+id+",-1);detail("+id+")\\">\\u2212</button><b style=\\"font-size:20px\\">"+q+"</b><button class=qbtn onclick=\\"chg("+id+",1);detail("+id+")\\">+</button></div>";' +
    'h+="<button class=place onclick=\\"detailAdd("+id+")\\">"+(q?("\\uD83D\\uDED2 View cart \\u00B7 "+inr(total())):("Add to cart"))+"</button>";' +
    'h+="<button onclick=closeSheet() style=\\"width:100%;background:none;border:none;color:#94a3b8;font-size:13px;padding:10px;margin-top:2px;cursor:pointer\\">Close</button>";' +
