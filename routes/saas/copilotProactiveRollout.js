/**
 * AI_ASSIST_ROLLOUT_v1 (2026-06-20) — flip on COPILOT_v4 "AI Assist"
 * (Proactive Sales Coach lead summary panel) for every tenant.
 *
 * Originally vserve-only (gated by COPILOT_PROACTIVE_ENABLED='1', flipped
 * by utils/cp4VserveAutoEnable.js). tenantBootstrap was updated to seed
 * COPILOT_PROACTIVE_ENABLED='1' on every NEW tenant. This module is the
 * one-shot bulk rollout for everything already provisioned.
 *
 * Mirrors quickNoteRollout — single config flag, no schema migration,
 * no per-lead recompute. Just UPSERT the flag and let the SPA do the rest.
 *
 * APIs (POST /api/saas with fn=…):
 *   api_saas_copilotProactive_rolloutPreview(token)
 *     → for each active tenant: is the flag already on? Returns counts.
 *   api_saas_copilotProactive_rolloutRun(token, { tenant_slug? })
...
async function _previewTenant(tenant) {
  ...
  try {
    const r = await pool.query(`SELECT value FROM config WHERE key = 'COPILOT_PROACTIVE_ENABLED'`);
...
async function api_saas_copilotProactive_rolloutPreview(token) {
...
async function api_saas_copilotProactive_rolloutRun(token, payload) {
  await requireSuperAdmin(token);
...
async function autoRolloutAtBoot() {
  try {
    ...
    console.error('[AI_ASSIST_ROLLOUT] boot rollout failed:', e.message);
  }
}

module.exports = {
  api_saas_copilotProactive_rolloutPreview,
  api_saas_copilotProactive_rolloutRun,
  autoRolloutAtBoot
};
