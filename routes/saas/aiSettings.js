...
async function api_saas_ai_settings_test(token) {
  ...
  const apiKey = decryptString(r.rows[0]?.gemini_api_key_enc);
  ...
  try {
    ...
    return { ok: true, models_visible: count, sample_model: j.models?.[0]?.name || null };
  } catch (e) {
