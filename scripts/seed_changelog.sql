...
SELECT * FROM (VALUES
  ...
  ('feature', '🔔 Mute notifications — per-user master switch',
   'Settings → Security now has a Notifications card with a simple ON/OFF button. When OFF, every push (new lead, follow-up due, heat alert, WhatsApp inbound, click-to-mobile, etc.) is silenced for that user only — other users are unaffected. Admins finally have a single-click way to go heads-down without uninstalling the app or disabling each tenant feature individually.',
   '#/admin', '🔔', NOW()),
...
,
  ...
  ('feature', 'Attach existing leads to a campaign in one click',
   'Until now the Lead match filter on a Campaign only governed FUTURE leads — anything already sitting in the database with the right Source / Status / Assigned-to had to be bulk-edited row by row. The campaign editor now has a clear toggle right under the match filter: (1) Future leads only — same as before, (2) Existing leads only — one-time backfill of what is already there, (3) Future + existing — both. When Existing or Both is picked, an inline condition builder appears. Pick a Match mode (ALL conditions = AND, ANY = OR) and any combination of Assigned-to (with an Unassigned sentinel), Status, and Source. A Preview button counts the matches before you commit. Crucially, there is an "Also unassign matched leads" checkbox — ticked by default — which clears assigned_to on the matched leads so they land back in the Pull pool. On Demand campaigns need this to work; without it the leads stay owned by whoever they belonged to and other agents cannot pull them. The backend is a single api_campaigns_attachExisting call, admin-only, that runs UPDATE leads SET campaign_id = X (and optionally assigned_to = NULL) WHERE the chosen filters hold. The campaign Save action calls it automatically after the campaign itself is saved. Solves the smcbroking diamond-lead scenario where a fresh campaign was set up but agents like Varsha saw "No free leads" because the existing admin-owned leads were never tagged to the campaign.',
   '#/campaigns', '🎯', NOW())
,
