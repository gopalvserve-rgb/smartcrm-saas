...
/**
 * INCOMING_CARD_v1 (2026-05-31) - Runo-style card shown on incoming call.
 *
 * Architecture: full-screen-intent notification launches this Activity.
 * UNLIKE the old CALL_OVERLAY_v1/v2/v3 (which drew on top of the dialer via
 * WindowManager and lost touch events to Vivo/Oppo), this is a real Activity
 * so the close button is just finish() - no flag warfare with any OEM dialer.
 *
 * Kill switch: SharedPreferences key "incoming_card_enabled".
 *   "1" or missing = enabled (default ON)
 *   "0"            = disabled (Activity finishes immediately)
 *
 * Auto-dismiss: 45 s, or when the user taps the X, or on deeplink tap.
 */
class IncomingCallActivity : Activity() {
    companion object {
        private const val TAG = "LeadCRM/InCard"
        private const val PREFS = "leadcrm"
        private const val KEY_ENABLED = "incoming_card_enabled"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isEnabled(this)) {
            Log.d(TAG, "incoming card disabled by user setting")
            finish()
            return
        }
