...
public class MainActivity extends BridgeActivity {
    ...
    public class LeadCRMBridge {
        ...
        /**
         * CALL_HISTORY_SYNC_v2 (2026-05-30) — backfill historical calls from
         * the phone's CallLog and POST each one to /api/call_event_native
         * (same endpoint PhoneStateReceiver uses for live events).
         *
         * Window: sinceMs..untilMs (Android's CallLog.Calls.DATE column is in
         * Unix ms — same epoch as System.currentTimeMillis()).
         * Types: incoming / outgoing / missed (mapped from CallLog.Calls.TYPE).
         *
         * Runs on a background thread so the WebView doesn't block. The JS
         * side awaits via a polling pattern OR just toasts a generic "started"
         * — we return immediately with a placeholder count, and each row
         * lands as an individual call_event the server processes async.
         *
         * Requires READ_CALL_LOG permission (granted via permission onboarding).
         */
        @JavascriptInterface
        public String importCallLog(final double sinceMs, final double untilMs) {
            final long since = (long) sinceMs;
            final long until = (untilMs > 0) ? (long) untilMs : System.currentTimeMillis();
            // Permission check
            if (ContextCompat.checkSelfPermission(MainActivity.this,
                    Manifest.permission.READ_CALL_LOG) != PackageManager.PERMISSION_GRANTED) {
                try {
                    JSONObject o = new JSONObject();
                    o.put("imported", 0); o.put("skipped", 0); o.put("total", 0);
                    o.put("error", "READ_CALL_LOG permission missing");
                    return o.toString();
                } catch (Exception e) { return "{\"error\":\"perm\"}"; }
            }
            final SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE);
            final String base = prefs.getString("api_base_url", "");
            final String tok  = prefs.getString("auth_token", "");
            if (base.isEmpty() || tok.isEmpty()) {
                try {
                    JSONObject o = new JSONObject();
                    o.put("imported", 0); o.put("skipped", 0); o.put("total", 0);
                    o.put("error", "no api_base_url / auth_token — log in first");
                    return o.toString();
                } catch (Exception e) { return "{\"error\":\"creds\"}"; }
            }

            // Do the heavy lifting on a worker thread, return a started ack.
            new Thread(() -> {
                int imported = 0, skipped = 0, total = 0;
                Cursor c = null;
                try {
                    String selection = CallLog.Calls.DATE + " >= ? AND " + CallLog.Calls.DATE + " <= ?";
                    String[] args = new String[] { String.valueOf(since), String.valueOf(until) };
                    ...
                    if (c == null) {
                        Log.w(TAG, "importCallLog: null cursor");
                        return;
                    }
                    total = c.getCount();
                    Log.i(TAG, "importCallLog: window " + since + ".." + until + " found " + total + " rows");
                    ...
                    int idxDuration = c.getColumnIndex(CallLog.Calls.DURATION);

                    while (c.moveToNext()) {
                        String number   = idxNumber   >= 0 ? c.getString(idxNumber) : "";
                        int    typeCode = idxType     >= 0 ? c.getInt(idxType)      : 0;
                        long   when     = idxDate     >= 0 ? c.getLong(idxDate)     : 0;
                        long   durSec   = idxDuration >= 0 ? c.getLong(idxDuration) : 0;

                        if (number == null || number.trim().isEmpty()) { skipped++; continue; }

                        // Map Android CallLog type → our string
                        // INCOMING_TYPE=1, OUTGOING_TYPE=2, MISSED_TYPE=3, REJECTED_TYPE=5
                        String direction;
                        boolean missed = false;
                        switch (typeCode) {
                            case CallLog.Calls.INCOMING_TYPE: direction = "incoming"; break;
                            case CallLog.Calls.OUTGOING_TYPE: direction = "outgoing"; break;
                            case CallLog.Calls.MISSED_TYPE:   direction = "incoming"; missed = true; break;
                            case CallLog.Calls.REJECTED_TYPE: direction = "incoming"; missed = true; break;
                            default: direction = "incoming"; break;
                        }

                        try {
                            JSONObject payload = new JSONObject();
                            payload.put("phone", number);
                            payload.put("direction", direction);
                            payload.put("event", missed ? "missed" : "ended");
                            payload.put("duration_seconds", durSec);
                            payload.put("started_at_ms", when);
                            payload.put("source", "call_log_backfill");

                            URL url = new URL(base + "/api/call_event_native");
                            ...
                            conn.setRequestProperty("x-auth-token", tok);
                            conn.setRequestProperty("Accept", "application/json");
                            byte[] body = payload.toString().getBytes("UTF-8");
                            conn.getOutputStream().write(body);
                            int code = conn.getResponseCode();
                            conn.disconnect();
                            if (code >= 200 && code < 300) imported++; else skipped++;
                        } catch (Exception e) {
                            skipped++;
                            Log.w(TAG, "importCallLog row failed: " + e.getMessage());
                        }
                    }
                    Log.i(TAG, "importCallLog done: imported=" + imported + " skipped=" + skipped + " total=" + total);
                } catch (Exception e) {
                    Log.e(TAG, "importCallLog fatal: " + e.getMessage());
                } finally {
                    if (c != null) try { c.close(); } catch (Exception ignored) {}
                }
            }, "importCallLog").start();

            // Return immediately. JS will see {started:true} and toast a generic message.
            try {
                JSONObject o = new JSONObject();
                o.put("started", true);
                o.put("imported", 0);
                o.put("skipped", 0);
                o.put("total", 0);
                o.put("note", "Backfill running in background — check Call Activity in a minute.");
                return o.toString();
            } catch (Exception e) { return "{\"started\":true}"; }
        }
