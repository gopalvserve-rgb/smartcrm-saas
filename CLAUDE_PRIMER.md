...
### Recent ships (2026-07-19)
- **`STOREFRONT_v1`** — self-serve WhatsApp store (WhatsApp Suite pack). Merchant builds a store + adds products 3 ways: manual, **from URL** (`api_wapack_product_from_url` scrapes og tags/JSON-LD/₹), **scan a menu photo** (`api_wapack_product_scan` → Gemini VISION; added `args.images` to `utils/geminiClient.generate`). Public customer page at `/t/<slug>/store` (routes `expressRenderStore`/`expressPlaceOrder` in server.js AFTER attachTenant, like /f/:formSlug). Orders recompute total server-side → create a lead (source Storefront) + drop into its remark + WhatsApp-notify the shopkeeper. Frontend VIEWS.wapackshop = 🏪 My Store / 📦 Products / 🧾 Orders. Scope: order→lead+WA note, COD/UPI note, NO gateway/Meta-catalog yet. Tables wapack_store + wapack_orders + wapack_products(+cols). 37-test harness `_wapack_store_test.js`. Cache whatsapp.js `2026-07-19-storefront-v1`. Verified live on vserve (left store INACTIVE). Commit 5866d86.
- **`PAGE_HELP_v1`** — per-page contextual Help. Self-contained `public/tenant/help.js` (floating ❓ button + drawer; content keyed off first hash segment; page-scoped, never navigates; "Show me" highlights a control by visible text; mobile-first). To edit help: the HELP{} object in help.js, no backend. Loaded from index.html injection block. Commit 287efbc.


- **`AIBOT_RESUME_PER_SETTING_v1`** (`routes/aiBot.js _shouldSuppress`) — the AI bot NEVER stops on a customer/agent READ (read receipts are UPDATE-only in whatsbot webhook: set read_at, insert no row). Gopal's "bot stops as soon as human reads" was really the ALWAYS-ON hardcoded 30-min human-handoff guard overriding the tenant's `resume_after_idle_seconds` (vserve=10s). FIX: guard window = `MIN(configured idle window, 1800s)` → vserve resumes in 10s, default tenants keep 30-min safety incl after-hours; no tenant less safe. Diagnose any thread with `api_aiBot_diagnose({phone})` (AIBOT_DIAG_v1 — richer than api_aibot_diagnose_thread). Manual "Pause until I resume" = ~100y pause via api_wb_botPause(minutes<=0); left as-is per Gopal. Commit afbbf7b.
- **`WA_PACK_BOT_FORM_v1`** (`routes/packs/whatsapp.js` + aiBot hook + `public/tenant/whatsapp.js`) — AI bot can send WhatsApp in-chat Forms. wapack_forms gains flow_id/cta_text/body_text/etc; new table wapack_bot_form_triggers (keyword→form, single row). `_sendForm` sends native Meta Flow (interactive type:'flow') when flow_id set, else a TEXT-PROMPT fallback (works w/o WABA Flow). `_botMaybeSendForm` is pack-gated via `framework.isPackActive('whatsapp')` → STRICT no-op for non-pack tenants; aiBot hook is a thin try/catch after `_shouldSuppress`. UI: forms editor "Native Flow" section + "AI Bot auto-send" panel + "Send test". NATIVE delivery needs a published Meta WhatsApp Flow ID. Installed WhatsApp pack on vserve (was pack-less; core nav intact). Cache key whatsapp.js `2026-07-19-botform-v1`. Commit fa26dc8.
- **DEPLOY LANDMINE:** the LOCAL `tenant sass/smartcrm-saas` folder can be BEHIND GitHub (local aiBot.js was missing Hinglish + api_aiBot_diagnose but still exported it → would crash). Deploy from a FRESH `git clone` + `diff`, never the local folder blind. `node -c` won't catch undefined-export ReferenceErrors.

### Recent ships (2026-06-20)
- **`AI_ASSIST_ROLLOUT_v1` + `AI_ASSIST_TUTORIAL_POPUP_v1`** — Promoted COPILOT_v4 AI Assist (Lead summary panel) from vserve-only to ALL tenants. New `routes/saas/copilotProactiveRollout.js` mirrors LS/QNote rollout pattern: preview + run super-admin APIs, `autoRolloutAtBoot()` UPSERTs `COPILOT_PROACTIVE_ENABLED='1'` on every active tenant at server boot (8s delay). `tenantBootstrap.js CONFIG_DEFAULTS` updated so new tenants ship with it ON. SPA side: `maybeShowAiAssistDailyTutorial()` in `public/tenant/app.js` near `restartProductTour` — gradient intro popup with 3 coloured-band sections, "Got it" + 7-day snooze, once-per-user-per-day key (`crm_ai_assist_tut_<uid>`). Boot hook at 4.5s. Cache: `2026-06-20-ai-assist-rollout`. Commit 1f49545.
- **`WA_THREADS_SCAN_v1` + `WA_THREADS_TABS_v1`** — WhatsApp Chats list. (1) `routes/whatsbot.js → api_wb_chat_threads`: scanLimit bumped 1000 → 10000, new `opts.show_all` path uses DISTINCT ON over the full whatsapp_messages table (one row per counter, no cap). Mobile path untouched. (2) `public/tenant/app.js → wbChat() renderThreadList` (~line 18127, NOT the team-chat one at 13075): Recent / History tabs (30-day cutoff). Persisted in `localStorage.crm.wb.activeTab`. Counts on each tab reflect post-search-filter pool. Cache key: `2026-06-20-wa-threads-tabs`. Commit 0971bb4.
