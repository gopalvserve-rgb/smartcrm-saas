...
const transactions = require('./routes/saas/transactions');
const emailLogs = require('./routes/saas/emailLogs'); /* EMAIL_LOG_v1 */
const marketplaceDiag = require('./routes/saas/marketplaceDiag'); /* TRADEINDIA_DIAG_v1 */ // SAAS_TXN_v1
...
require('./routes/packs/whatsapp'); // WHATSAPP_PACK_v1 (2026-07-18)
...
[
  ...
  emailLogs,  /* EMAIL_LOG_v1 */
  marketplaceDiag   /* TRADEINDIA_DIAG_v1 */
].forEach(mod => {
 ...
 * ...which returns the FIRST tenant (ORDER BY id ASC) whose users table has that id.
 * User ids are PER-TENANT SERIALS (id 3 is Babita in `jay`, Neetu in `vserve`), so a
 * rep's upload could be written into a DIFFERENT COMPANY'S DATABASE.
 *   1. `t` claim in the token → the tenant is STATED. Verify the user is in it.
 *   2. Legacy token (no `t`)  → look up on id AND EMAIL, not id alone.
 *   3. Still ambiguous        → REFUSE. A 404 beats writing into the wrong CRM. */
async function _findTenantForUser(decoded) {
  ...
  for (const row of r.rows) {
    ...
    try {
      const hit = email
        ? await pool.query(
            `SELECT 1 FROM users WHERE id = $1 AND LOWER(COALESCE(email,'')) = $2
  ...
  return tenantDb.tenantStorage.run({ pool: req.tenantPool, tenant: req.tenant, slug: req.tenantSlug },
    async () => {
      try {
         ...
         * Was `if (!leadId && phone)`: a client-supplied lead_id won even when the recording's
         * own number said otherwise. The client's lead came from a TIME match and the phone
         * from a different pick, so they could disagree — 108 recordings landed on the wrong
         * customer. If we have a number, the lead it belongs to IS the lead. */
        if (phone) {
         ...
         * Was: ON CONFLICT (user_id, dedup_key) DO NOTHING. But the unique index this code
         * self-heals is PARTIAL (WHERE dedup_key IS NOT NULL), and Postgres cannot infer a
         * partial index from a bare ON CONFLICT (cols) — it raises "no unique or exclusion
         * constraint matching the ON CONFLICT specification". That was SWALLOWED by
         * catch{console.error}, leaving id=null. Every upload on tenant `jay` failed (41/41)
         * while `vserve` worked (older non-partial index) — invisible on our test tenant.
         * Fix: don't depend on the index SHAPE. Plain INSERT ... RETURNING id; a real
         * duplicate (SQLSTATE 23505) is handled as the race it is; the true error is reported. */
